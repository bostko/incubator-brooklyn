...
public class CreateUserPolicy extends AbstractPolicy implements SensorEventListener<Location> {
    ...
    protected void addUser(Entity entity, SshMachineLocation machine) {
        ...
        int result = machine.execScript(ImmutableMap.of(SshTool.PROP_RUN_AS_ROOT.getName(), true), "create-user-"+user, ImmutableList.of(BashCommands.addSbinPathCommand(), cmd));
        if (result != 0) {
        ...
        result = machine.execScript(ImmutableMap.of(SshTool.PROP_RUN_AS_ROOT.getName(), true), "create-user-"+user, ImmutableList.of(BashCommands.addSbinPathCommand(), cmd));
        if (result != 0) {
            throw new IllegalStateException("Failed to enable ssh-login-with-password, using command "+ BashCommands.addSbinPathCommand()+"\n"+cmd);
        }

        // Exec command to grant sudo rights.
        if (grantSudo) {
            List<String> cmds = ImmutableList.of(
                    BashCommands.addSbinPathCommand(),
